...
import type { StageName } from "@/lib/models";
import {
...
export async function POST(req: NextRequest) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      let run;
      try {
        const pipeline = new QAPipeline(settings, (ticketKey, stage, status, message) => {
          send("progress", { ticketKey, stage, status, message });
        });
